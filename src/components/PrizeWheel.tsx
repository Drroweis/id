...
const PrizeWheel: React.FC = () => {
  ...
  // Fetch initial spin stats when component mounts
  useEffect(() => {
    const fetchSpinStats = async () => {
      try {
        const response = await fetch(endpoints.game.stats, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
          },
        });
        
        if (!response.ok) throw new Error('Failed to fetch spin stats');
        
        const data = await response.json();
        setSpins(data.spinsLeft);
        
        // Set recovery time if spins are depleted
        if (data.spinsLeft === 0 && data.nextRefreshTime) {
          setRecoveryTime(new Date(data.nextRefreshTime).getTime());
        }
      } catch (error) {
